...
public class AmqpsProvisioningSaslHandler implements SaslHandler
{
    ...
    public void handleOutcome(SaslOutcome outcome)
    {
        ...
        switch (outcome)
        {
            ...
            case AUTH:
                ...
                throw new SecurityException(new ProvisioningDeviceSecurityException("Sas token was rejected by the service"));

            case SYS_TEMP:
                // Codes_SRS_AMQPSPROVISIONINGSASLHANDLER_34_022: [If the sasl outcome is not OK, this function shall throw a SecurityException.]
                throw new SecurityException(new ProvisioningDeviceSecurityException("Sasl negotiation failed due to transient system error"));
            ...
            default:
                ...
                // Codes_SRS_AMQPSPROVISIONINGSASLHANDLER_34_022: [If the sasl outcome is not OK, this function shall throw a SecurityException.]
                throw new SecurityException(new ProvisioningDeviceSecurityException("Sasl authentication against service failed"));
        }
    }
    ...
    private byte[] handleThirdChallenge(byte[] challengeData)
    {
        ...
        try
        {
            ...
            this.responseCallback.run(new ResponseData(this.challengeKey, ContractState.DPS_REGISTRATION_RECEIVED, 0), authorizationCallbackContext);
        }
        ...
        while (this.sasToken == null && millisecondsElapsed < MAX_MILLISECONDS_TIMEOUT_FOR_SAS_TOKEN_WAIT)
        {
            ...
            catch (InterruptedException e)
            {
                ...
                {
                    throw new SecurityException(new ProvisioningDeviceSecurityException("Sasl negotiation failed"));
                }
            }
        ...
        if (millisecondsElapsed >= MAX_MILLISECONDS_TIMEOUT_FOR_SAS_TOKEN_WAIT)
        {
            ...
            throw new SecurityException(new ProvisioningDeviceSecurityException("Sasl negotiation failed: Sas token was never supplied to finish negotiation"));
        }
